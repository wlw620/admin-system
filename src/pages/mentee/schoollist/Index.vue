...
<template>
  <div class="container-box">
        ...
        <Form :mode="formData" label-position="top">
          <Col span="10" class="col">
            <FormItem :label="$t('addschool.schoolnameTit')">
              <Select
                :model="formData.schoolid"
                :placeholder="$t('addschool.choose')"
                filterable
                clearable
              >
                <Option
          ...
          <Col span="4" class="col">
            <Button class="buttom" type="primary" @click="addSchool">{{$t("addschool.submitTxt")}}</Button>
          </Col>
        </Form>
      </Card>
    </Row>
    <Row>
      <Card :bordered="false" class="margin-b-20">
        <Table :columns="columns" :data="schoollist"></Table>
      </Card>
    </Row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getMainData() {
      ...
      // this.listData = data.selectedschools;
